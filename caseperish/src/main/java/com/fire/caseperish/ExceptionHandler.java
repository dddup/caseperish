...
public abstract class ExceptionHandler {
    ...
    final void bandageExceptionHappened(Thread t , Throwable throwable) {
        try {//捕获监听中异常，防止使用方代码抛出异常时导致的反复调用
            if(ExceptionFilters.filters(throwable)){
                return;
            }
            onBandageExceptionHappened(t,throwable);
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    /**
     * 子线程抛出异常时始终调用该方法。主线程只有第一次抛出异常时才会调用该方法，该方法中到的throwable都会上报到bugly。以后主线程的异常只调用 {@link #onBandageExceptionHappened(Thread, Throwable)}
     *
     * @param thread
     * @param throwable
     */
     ...
     */
    protected abstract void onBandageExceptionHappened(Thread t, Throwable throwable);
